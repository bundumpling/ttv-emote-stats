...
const beforeEnterRouteWithEmoteID = (
  to: RouteLocationNormalized,
  from: RouteLocationNormalized,
  next: NavigationGuardNext
) => {
  if (validateEmoteID(to.params.emoteID)) {
    document.title = `${baseTitle} | Emote | ${
      String(to.params.emoteID).split("-")[1]
    }`;
    next();
  } else {
    next({ name: "NotFound" });
