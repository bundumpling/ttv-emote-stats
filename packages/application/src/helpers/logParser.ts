...
export default async function logParser(log: string, emotes: string[]) {
  ...
  const lineparserRegExp =
    /^\[(\d{2}:\d{2}:\d{2})\]\s{1,2}(?:(?!Uploading|Unrecognized)(?:[^:\s]+\s))?(\w+)(?::)\s(.+)\n$/;
  ...
  const setUsernameLastSeen = (username: string, timestamp: number) => {
    if (timestamp) {
      usernameLastSeen[username] = usernameLastSeen[username]
        ? Math.max(usernameLastSeen[username], timestamp)
        : timestamp;
    }
  };
